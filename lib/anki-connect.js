...
<div class="card front">
  ...
  <div class="footer">AnkiTrans</div>
</div>
`.trim();
...
.footer {
  ...
  font-weight: 300;
  letter-spacing: 1px;
  border-top: 1px dashed var(--border); /* 强调分割线 */
  opacity: 0.6;
  user-select: none;
}
...
export async function ensureAnkiTransModel() {
  ...
  if (!exists) {
    ...
    // 如果模型已存在，尝试更新其模板和样式以确保是最新的
    try {
      await invoke('updateModelTemplates', {
        model: {
          name: ANKITRANS_MODEL_NAME,
          templates: {
            'AnkiTrans Card': {
              Front: ANKITRANS_FRONT_TEMPLATE,
              Back: ANKITRANS_BACK_TEMPLATE,
      ...
      console.log('AnkiTrans model templates and styling updated.');
    } catch (err) {
      console.warn('Failed to update AnkiTrans model:', err);
    }
    return false;
  }
}
...
/**
 * 上传媒体文件到 Anki
 * @param {string} filename - 文件名 (如 "audio.mp3")
 * @param {string} data - Base64 编码的文件内容
 * @returns {Promise<string|null>} - 成功返回文件名，失败返回 null (AnkiConnect 返回 null 表示成功, 但如果有 error 会抛出)
 */
